...
** TODO Work In progress
   ...
   - Cross validation to slow
   - Testing statistic not well calibrated on generative dataset

* COMMENT debrief
  
** Pk pas du non linéaire
   - il faut des algos efficaces
   - la non linéarité c'est pas une propriété !! On prend quoi comme model ?
   - On ratisse large ! c'est de la fouille de données, on donne une liste de
     candidats !!
   - Model linéaire c'est un dev de taylor a l'ordre 1 ;D, on est dans une
     approximation qui va deja capter de la variance ! On gagnerais peut être un
     petit peu avec un non liéaire !!
   - Ma méthodo s'inscrit dans une grande pipeline ! faut voir que c'est blindé
     d'approximation avant et après ! On ratisse large ! mais je l'ai deja dit ! 

 bibliography:biblio.bib
 bibliographystyle:apalike

** Expliquer plus les mecanismes d'ataptation
   - il faut mieux expliquer les phénomène d'adaptation a l'env surtout a un
     publique non expert !
** Variance inter est plus forte que la variance intra ! 
   - c'est pas pour ca que on peut pas faire d'analyse factorielle !
   - C'est un peut comme si on avait beacoups de bruit ... mouais a reflechir !
   - Il a peut etre plus de variance entra que inter Mais l'acp capte les axe de
     plus forte variance du coup quand on met des Europeen avec des Africains ba
     PC1 ce met dans le sens qui cluster les 2 pops car le reste c'est du bruit
     !!! Les variances intra peuvent etre très très grande c'est pas ce qui
     structure les données dans sa totalité ! A voir !! 
 #  LocalWords:  Genotypic
