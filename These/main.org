...
each 2 hyper-parameters which can be cross-validated. We propose other procedure
to assess hyper-parameters that gave good results in our experiments.

****** ANNEX Cross validation algorithm
...
Analysis (PCA), we can seen them as a PCA of $$G - X^B$$ if we know the primary
effect matrix. Thus we propose to select the number of latent variable $\K$
by visualizing the scree plot.
...
estimate latent variation while protecting variation explain by co-variate $X$,
...
***** TODO Heuristic to choice of $\lambda$ lasso
:LOGBOOK:
...
value inspired by cite:friedman10_regul_paths_gener_linear_model. We start with
the smallest value of $\lambda$ such that the entire vector
$$ \hat(B) = argmin \norm{(\Y - \C_{t = 0}) - \X \B^{T}} + \lambda |B|$$ 
equal zero.
...
Until now, we only presented how to estimate latent factor scores and loadings
and primary effects. However, the purpose which motivate the estimation of
latent factor by considering the primary variable is the detection of observed
output variables associated with the variable of interest. We propose here to
use latent factor score matrix $U$ as co-variables of a linear model.
...
We write the linear model for each observed output variable $\Y_{j}$ as follow 
\begin{equation}
\Y_{j} =  \U \matr{\gamma_{j}}^{T} + \X \beta_{j} + \matr{\epsilon_{j}}
\end{equation}

where \matr{\epsilon_{j}} 
