...
:END:
*** STARTED Preprocessing genotype data
:LOGBOOK:
...
#+begin_src R :results output :exports both :session *ssh patator*
  ...
  G <- G[rownames(G) %in% indiv.df$sample,]
  dim(G)
...
#+begin_src R :results output :exports both
  ...
  ## compute coodinates for each indiv
  TODO

  ## compute X
  climate <- getData('worldclim', var='bio', res = 2.5)
  bio <- extract(climate, y = as.matrix(coordinates))
...
**** STARTED with 0.05 0.5 1 5 % outlier and L = 100 000
:LOGBOOK:
...
:END:
***** RUNNING Run without lfmm Lasso
:LOGBOOK:
...
**** STARTED lfmmLasso + pca
:LOGBOOK:
- State "STARTED"    from "DEBUG"      [2017-05-23 mar. 15:45]
- Note taken on [2017-05-23 mar. 15:45] \\
  on va voir si ca, ca plante
...
#+begin_src R :results output :exports both
  ...
  func <- lm_zscore(calibrate = FALSE,
                    sigma.computation = "lm",
                    correctionByC = FALSE)$fun
  expr <- MethodBatchExperiment("Celiac whole dataset",
                                s = Article3_Celiac_sampler(FALSE),
                                method.batch = expr$method.batch,
                                func = func)
  expr <- runExperiment(expr, save = TRUE)
#+end_src
**** TODO lfmmLasso + pca
...
#+begin_src R :results output :exports both
## cate
method.batch$m.cate <- finalcateMethod(K = 9)
#+end_src
