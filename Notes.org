...
#+RESULTS:
#+begin_example
  DEBUG [2017-05-29 13:18:30] We set the seed said:
  --------------------------------

  --------------------------------
  Warning messages:
  1: executing %dopar% sequentially: no parallel backend registered
  2: In matrix(cate.results$Z, n, m$K) :
    la longueur des données [136395] n'est pas un diviseur ni un multiple du nombre de lignes [94497]
#+end_example

Il y a eu un warning bizare... On passe a la suite on vera bien ! 
