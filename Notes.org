...
#+RESULTS:
[[file:/home/cayek/Projects/Thesis/Rfigures/figure-12505znD.png]]
** STARTED Comparaison of methods for 3Article                    :3Article:
:LOGBOOK:
...
**** Do we find these loci with lfmmLasso and lfmmRidge
#+begin_src R :results output :session *R* :exports both
  exp.calibrated <- Article3_runExp_calibrate(exp)

  Article3_runExp_manhattan(exp.calibrated, 0.05, "refactor")
  Article3_runExp_hist(exp.calibrated, 0.3, "refactor")

  pval <- (exp.calibrated$df.res %>% dplyr::filter(K == 2, method == "refactor"))$pvalue
  length(pval)
  hist(pval)
  ftools <- fdrtool::fdrtool(pval, statistic = "pvalue")
  hist(ftools$pval)
  hist(ftools$qval)


  exp.aux <- exp
  exp.aux$df.res <- exp.aux$df.res %>% dplyr::filter(K == 2, method == "refactor")
  exp.aux <- Article3_runExp_calibrate(exp.aux)

  auwf <- function(pvalue) {
    message(length(pvalue))
    res <- fdrtool::fdrtool(as.numeric(pvalue), plot = FALSE, statistic = "pvalue")
    tibble(pvalue = res$pval, qvalue = res$qval)
  }
  rr = exp.aux$df.res %>% group_by(K, lambda, sparse.prop, method) %>%
    dplyr::do(auwf(.$pvalue))
  rr
  hist(rr$pvalue)
#+end_src
...
- Note taken on [2017-04-10 lun. 17:40] \\
  ...
  ne met pas de var environmental) et fdr controlé !!
...
#+begin_src R :results output :session *R* :exports both
  ...
  ## experiment
  exp <- do.call(FDRControlExperiment, c(list(nb.rep = 1,
                                              sampler = s), methods))
