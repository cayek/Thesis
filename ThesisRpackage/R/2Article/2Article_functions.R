...
Article2.env$fst.LEA <- function(project,run = 1, K, ploidy = 2){
  #require(LEA)
...
Article2.env$tess3old.wrapper <- function(data.list, K, alpha) {
...
Article2.env$tess3.wrapper <- function(data.list, K, method) {
  if (is.null(data.list$admixed.genotype)) {
...
Article2.env$snmf.wrapper <- function(data.list, K, alpha = 10) {
...
Article2.env$beforeadmixture.wrapper <- function(data.list) {
  res <- list()
...
Article2.env$sample.data <- function(simu.param) {
...
Article2.env$sampler.from.data <- function(X, coord) {
  function(n, L) {
...
Article2.env$dataframeFst <- function(fst, outlier, ...) {
...
Article2.env$AUC.precision.recall <- function(fst, outlier) {
  df <- data.frame(index = 1:length(fst), fst = fst, outlier = 1:length(fst) %in% outlier)
  m <- length(fst)
  m_0 <- length(outlier)
  # sort fst
...
Article2.env$fig3.experiment <- function(simu.param, m.ms, rep) {
...
Article2.env$fig1.exp.n <- function(simu.param, ns, rep) {
...
Article2.env$fig1.exp.L <- function(simu.param, Ls, rep) {
...
Article2.env$fig4.aux <- function(res, method, n, L, r) {
...
Article2.env$fig4.exp.n <- function(sample.data, ns, rep, L, K, tess3Old.alpha = 0.03) {
...
Article2.env$fig4.exp.L <- function(sample.data, Ls, rep, n, K, tess3Old.alpha = 0.03) {
