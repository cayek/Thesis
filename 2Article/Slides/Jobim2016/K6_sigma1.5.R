...
load(paste0(res.dir,"tess3.run.K6.sigma1.5.weightheat.res"))

################################################################################
# Q plots


Q <- tess3.run.K6.sigma1.5.weightheat$Q
# northen cluster
...
pdf(paste0(fig.dir,"Q_K6_sigma1_5.pdf"), width = slide$width * 0.8,height = slide$heigth * 0.8)
par(mar = c(0,0,0,0))
layout(matrix(1:2, nrow = 2, ncol = 1), heights = c(6,1))
par(mar = c(0,0,0.5,0))
plot(Q = tess3.run.K6.sigma1.5.weightheat$Q, 
...
################################################################################
# G plots
p <- tess3.run.K6.sigma1.5.weightheat$pvalue
class(p) <- "numeric"
toplot <- data.frame(fst = tess3.run.K6.sigma1.5.weightheat$Fst, 
                     pvalue = p,
                     call_method_75_TAIR8.europe$locus.coord, 
                     index = seq_along(tess3.run.K6.sigma1.5.weightheat$Fst))
