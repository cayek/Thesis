...
\begin{algorithm}[H]
 ...
 \KwOut{the admixture matrix ${\bf Q} \in \mathbb{R}^{n \times K}$, the ancestral genotype frequency matrix ${\bf G} \in \mathbb{R}^{K \times (p+1) L}$ }
...
\begin{algorithm}[H]
  \KwIn{ the data matrix ${\bf Y} \in \{0,1\}^{n \times (d+1) L}$, the
    eigenvalues matrix ${\bf \Delta}$ and eigenvectors matrices ${\bf U}$ such
    that ${\bf \Lambda} = {\bf U}^T {\bf \Delta} {\bf U}$, the number of
    ancestral populations $K$, the regularization coefficient $\alpha$, the
    maximum number of iteration $itMax$}

\KwOut{the admixture matrix ${\bf Q} \in \mathbb{R}^{n \times K}$, the ancestral genotype frequency matrix ${\bf G} \in \mathbb{R}^{K \times (d+1) L}$ }
